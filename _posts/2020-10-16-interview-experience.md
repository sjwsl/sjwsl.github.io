...
C++ 中什么情况会段错误，什么是未定义行为，未定义行为什么时候会段错误？涉及到虚拟内存和物理内存，例如访问野指针，如果地址对应的虚拟内存已经被分配了物理内存，就不会段错误，但后果未知。如果虚拟内存还没有分配物理内存，就会段错误。然后追问了虚拟内存物理内存的关系，虚拟内存的作用。

两个核并行做 a++ 100 次最后的结果可能是多少？[2,200]，上界很显然，推出下界的情况就行。四个核呢？[2,400]

谈谈如何写出操作系统友好的程序。充分利用缓存（例如上面的遍历二维数组），减少内存分配等等。

怎么避免内存越界、泄漏、溢出等错误。用智能指针，不用裸指针，用 STL，不用 raw array。善用工具调试，如 gdb，valgrind。

如何利用多级流水线优化程序。我说我不知道多级流水线，面试官和我讲了一下。在启发下我讲了一下我的理解和实际应用，最后答得还行。
...
工作之外的个人项目，讲了下自己平时维护的 C++ 基础库，具体介绍了下自己包的 slice，面试官听完后说这就是 C++17 的 stringview。。

**英语**

英文介绍下自己，主要介绍技术方面，然后英文简单交流一下。之前不知道有英语环节，不过内容挺简单的。

**闲聊**

因为面试的轮数太多了，每轮最后都有闲聊环节。主要聊了以后的打算，实习时间安排，工作内容介绍等等。到后面我都实在找不到问题问了。。

大三下的日常实习我只投了 N 厂，主要是学期末太忙，还要准备 Google 的面试。面试的体验除了太长都挺好的，没太有八股文，C++ 和操作系统的问题都比较深，有些问题我答得也不好。面试官都很关注我的能力、兴趣和他们的工作是否 match，最后一面大 boss 又详细介绍了他们的各种工作，反复确认我是否真的感兴趣。于是其他厂也懒得投了，又拒掉了之前答应的字节（不想 996），接了 N 厂的 offer。工作主要会是 C++，做用于 GPU 测试的不同层次 simulator。N 厂的 C++ 水平应该还是不错的（面试官说生产环境会用到 C++17），希望能学到些东西。实习待遇有点差，本硕博区别对待，但很舒服，标准的 965。

update 2021.3.4: 入职后我摸鱼摸成了 1164

## Akuna Capital (2021.3 挂了)

**一面**

英文面，主要就是做题，然后让我提问。

手算两个十六进制数与。每位拆成四个二进制位做即可（开始没看见是十六进制。。）

二叉树中最大值。弱智。

实现一个只支持 `push_back` 和按下标取元素的 `vector`。简单包一下 raw array 就行。

挺简单的。。面完三分钟就收到邮件通过了。

**二面**

中文面。实现一个 `event_timer`，支持添加定时事件以及批量执行。用优先队列写了一个，大概没问题吧。 

**三面**

英文面。开始一直在问 C++ 的异常，我一点也不会，面试官怎么引导也说不上来。。然后放弃了，问我算法题，二叉树LCA，秒了。

**四面**

HR 面。问我之后打算，有没有别家 offer。因为这时候有 Google offer 了，我就直接说明白不怎么想去。然后就给我挂了，还送了个杯子（进二面的都有）。
